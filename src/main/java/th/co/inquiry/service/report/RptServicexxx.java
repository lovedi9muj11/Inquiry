...
public class RptServicexxx extends BaseExcelRptService {
	...
	public Workbook getReport(Workbook workbook, List<ReportBean> entity, ReportBean bean) {
		...
		header.setCenter(sh.getHeader().getCenter().concat("ประจำวันที่ "+" ").concat((formatter2.format(new Date())+" ").concat("ถึงวันที่".concat(" ").concat(""+formatter2.format(new Date())))));
