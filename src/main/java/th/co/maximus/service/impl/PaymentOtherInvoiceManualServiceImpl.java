	...
	public void insertPaymentInvoiceManual(PaymentOtherFirstBean paymentBean, int userId) {
		...
		if (!paymentBean.getDocumentNo().equals("")) {
			for (int i = 0; i < paymentBean.getPaymentBill().size(); i++) {
				...
				//paymentInvoiceManualBean.setDiscountspecial(paymentBean.getSale());
				...
				paymentInvoiceManualBean.setAmount(paymentBillBean.getSummarySale().doubleValue());
				
				paymentInvoiceManualBean.setBeforVat(paymentBillBean.getSummarySale().doubleValue() - paymentBillBean.getVatSale().doubleValue());
