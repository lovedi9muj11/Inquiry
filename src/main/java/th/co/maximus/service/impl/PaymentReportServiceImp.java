...
//			resultBean.setDepartment(masterDataDao.findProperty(resultBean.getDepartment()));
	...
	public List<ReportPaymentBean> findPaymnetReportServicePDF(ReportPaymentCriteria criteria,String serviceType) throws Exception {
		...
		result = paymentManualDao.getReportPaymentPDF(criteria, serviceType);
...
//						refno.add(code.substring(12, 16));
			...
			resultBean.setPaymentMethod(paymentCodeRes);
			resultBean.setServiceCode(resultBean.getServiceCode());
			resultBean.setDepartment(resultBean.getDepartment()+masterDataDao.findProperty2(resultBean.getDepartment(), resultBean.getDepartmentArea()));
			data.add(resultBean);
		}
	...
	public List<ReportPaymentBean> findPaymnetReportServiceOtherSearch(ReportPaymentCriteria criteria,String serviceType) throws Exception {
		...
		for(ReportPaymentBean resultBean : result) {
			...
			for (int i = 0; i < methodResult.size(); i++) {
				...
				if (stockObject.getCode().equals("CC") && chkCC) {
					chkCC = false;
					payCode = "เงินสด";
					results.add(payCode);
