...
public class OfflineBatch implements Job {
    ...
    public CronTriggerFactoryBean sampleJobTrigger5(@Qualifier("jobWithSimpleTriggerBean") JobDetail jobDetail) throws Exception {
    	MasterDatasBean masterDatas = masterDatasDao.findByGrop(Constants.MasterData.TRIGGER_GOUP, Constants.MasterData.KEYCODE.TRIGGER_CLEARING);
    	return ConfigureQuartz.createCronTrigger(jobDetail, masterDatas.getValue());
    }
    ...
    public void saveBatch() throws Exception {
		...
		if (result != null) {
			...
			for (PaymentMMapPaymentInvBean payment : result) {
				clearingPaymentEpisOfflineService.updateStatusClearing(payment.getManualId(), Constants.CLEARING.STATUS_Y);
			}
		}
