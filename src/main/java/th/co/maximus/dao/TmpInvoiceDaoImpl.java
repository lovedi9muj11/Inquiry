...
public class TmpInvoiceDaoImpl implements TmpInvoiceDao{
	...
	public void insertTmpInvoice(TmpInvoiceBean tmpInvoiceBean) {
		StringBuilder sql = new StringBuilder();
		 sql.append("INSERT INTO tmpinvoice (MANUAL_ID, INVOICE_NO,INVOICE_DATE,DATE_LINE,BEFOR_VAT,VAT_AMOUNT,AMOUNT,VAT_RATE, CUSTOMER_NAME, CUSTOMER_ADDRESS, CUSTOMER_SEGMENT, CUSTOMER_BRANCH, TAXNO, ACCOUNTSUBNO, PERIOD ,CHANG, CREATE_BY, CREATE_DATE,UPDATE_BY,UPDATE_DATE,RECORD_STATUS)  ");  
		 sql.append(" VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)  ");
		 ...
		 jdbcTemplate.update(sql.toString(), tmpInvoiceBean.getManualId(),tmpInvoiceBean.getInvoiceNo(),tmpInvoiceBean.getInvoiceDate(),tmpInvoiceBean.getDateLine()
				 ...
				 ,tmpInvoiceBean.getPeriod(),tmpInvoiceBean.getChange(),tmpInvoiceBean.getCreateBy(),tmpInvoiceBean.getCreateDate(),tmpInvoiceBean.getUpdateBy(),tmpInvoiceBean.getUpdateDate(),tmpInvoiceBean.getRecordStatus()
