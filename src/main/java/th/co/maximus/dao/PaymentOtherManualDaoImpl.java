...
//		PaymentResultReq beanReReq = new PaymentResultReq();
		...
		try {
			StringBuilder sqlStmt = new StringBuilder();
			sqlStmt.append("SELECT  (select  REVENUE_TYPE_NAME from MAP_GL_SERVICE_TYPE where  REVENUE_TYPE_CODE = py.AMOUNTTYPE AND PRODUCT_CODE = py.SERVICECODE  GROUP BY REVENUE_TYPE_NAME) as AMOUNTTYPE");
